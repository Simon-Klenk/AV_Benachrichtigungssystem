...
class Writer:
    ...
    def _get_char(self, char, recurse):
        ...
        self.char_width = char_width  # <<< KORREKTUR 1 (VON UNTEN VERSCHOBEN)
    ...
    def _printchar(self, char, invert=False, recurse=False):
        s = self._getstate()
        self._get_char(char, recurse) # Setzt self.glyph und self.char_width

        # --- START KORREKTUR 1 (falsche Zeichen) ---
        if self.glyph is None and not recurse and (char == '\n' or char == '\t'):
            return

        if self.glyph is not None:
            # --- OPTIMIERUNG (RAM / GC) ---
            # buf = bytearray(self.glyph) # <-- ALT: Neue Zuweisung/Kopie
            buf = bytearray_at(addressof(self.glyph), len(self.glyph)) # <-- NEU: Memory View
            # --- Ende Optimierung ---
            if invert:
